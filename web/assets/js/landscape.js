...
(function (config) {
    ...
    var player = undefined,
        ...
        oldy = undefined,
        stage = undefined;
    var rotShift = config.config.fov / config.config.resx;
    var rayShift = config.config.mapw / config.config.resx;
    ...
    function getRGB() {
        ...
        var containerbg = new createjs.Shape(); // background colour for container
        ...
        for (var a = 0; a < config.config.resx; a++) {
            var _oldx = player.x;
            var _oldy = player.y;
            var calcrot = 0 - player.rotation + 180 - config.config.fov / 2 + a * rotShift; // this is calculating the rotation shift of the current ray in the loop. determined by fov, player rotation and resolution
            for (var b = 0; b < config.config.steps; b++) {
                var getCoords = getLineEndCoords(_oldx, _oldy, config.config.depth / config.config.steps, calcrot); // for each of the rays x amount of steps are made 'along the ray' and..
                var _imageData = gctx.getImageData(getCoords[0], getCoords[1], 1, 1); // ..on each position pixel data is read from the bitmap
                var height = _imageData.data[0]; // since heightmap holds gray scales only (colour value 0-255), take just one of the rgb values, they will always be equal. stick with [0], which is red and declare it as height
                rayArray.push(height); // store height in array
                _oldx = getCoords[0]; // next step is calculated from current x,y, so the old coords need to be stored in a var
                _oldy = getCoords[1];
            }
            rayArray.reverse(); // reverse the array, draw shapes first that are in the background
            for (var c = 0; c < rayArray.length; c++) {
                // for each 'step' a shape is created, set to the size of the height that was read from the bitmap
                var rayGfx = new createjs.Shape();
                var blue = -150 - rayArray[c];
                if (blue < 0) {
                    blue = 0;
                }
                var green = 255 * rayArray[c];
                var red = 65536 * 55;
                var color = (blue + green + red).toString(16); // quick conversion to hex
    ...
    function getLineEndCoords(originX, originY, hypotenuse, angle) {
        // feed it with angle and length of the 'opposite' (longest side) and it will calculate matching x,y coords.
        // todo: import this
        var x = originX + Math.sin(angle * (config.config.pi / 180)) * hypotenuse;
        var y = originY + Math.cos(angle * (config.config.pi / 180)) * hypotenuse;
        return [x, y];
    }
})(appConfig);
